...
folder_seg <- paste0(folder_data, "trichome/segmented/run3/") # input
...
clean_pixsets <- function (file_name, file_name_extracted, save_imt = T) {
    ...
    img_seg <- image_read(paste0(folder_seg, file_name)) %>% magick2cimg()
    save.image(img_seg, paste0(folder_temp, file_name_extracted, "/01-seg.png"))
    cat("\tSegmentation")

    # 2. Threshold
    img_bin <- threshold(img_seg, .5) %>% as.cimg()
    save.image(img_bin, paste0(folder_temp, file_name_extracted, "/02-threshold.png"))
    cat("\tBinary")

    # 3. Create a new binary image containing only large objects
    img_large <- remove_small(img_bin, 1e4)
    save.image(img_large, paste0(folder_temp, file_name_extracted, "/03-large.png"))
    ...
    save.image(img_mask, paste0(folder_temp, file_name_extracted, "/05-mask.png"))
    ## Subset the pixels
    ps_tri <- (img_seg * img_mask) > 0.8
    img_tri <- as.cimg(ps_tri)
    save.image(img_tri, paste0(folder_temp, file_name_extracted, "/06-trichome.png"))
    ## Remove small pixsets
    img_tricle <- clean(img_tri, 2) %>% as.cimg()
    save.image(img_tricle, paste0(folder_temp, file_name_extracted, "/07-trichome_cleaned.png"))
